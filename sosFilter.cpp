...
void sosFilter_c(double *sos, int nsec, double gain, double *x, int nx, double *zi, double *y) 
{
  ...
  {          
    ...
    for (int r = 0; r < nsec; r++)
    {
      //NumericVector sectRow = NumericVector(6);
      //for(int k = 0; k < 6; k++)
      //   sectRow[k] = sos(r, k);
      //double * sectRow = sos + r * 6;
      //if(i==0)
      //{
      //  for(int k = 0; k < 6; k++)
      //    printf("%g, ", sectRow[k]);
...
NumericVector sosFilter_cpp(NumericMatrix sos, double gain, NumericVector x, NumericMatrix zi) 
{
  ...
  {          
    ...
    for (int r = 0; r < nsec; r++)
    {
      ...
      value = sectionFormIItrans(state + r * 3, &sectRow[0], value);
    }
    y[i] = value * gain;
  }
  */
